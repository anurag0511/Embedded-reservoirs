...
#set the weight matrices for y axis values
W2 = (np.random.rand(resSize,resSize)*2)-1
Wprob2 = np.random.rand(resSize,resSize)
W2[Wprob2<0.7]= 0

Wfb2 = (np.random.rand(resSize,outsize)-0.5)*2
Wfbprob2 = np.random.rand(resSize,outsize)
Wfb2[Wfbprob2<0.7] = 0

Wout2 = (np.random.rand(outsize,resSize)-0.5)*2

Win2 = (np.random.rand(resSize,inputsize)-0.5)*2
Winprob2 =np.random.rand(resSize,inputsize)
Win2[Winprob2<0.7] = 0

Wdel2 = zeros((outsize,resSize))

#spectral radius 
rhoW = max(abs(linalg.eig(W)[0]))
W *= 1.25/rhoW
#spectral radius for y set 
...
while temp < 100:
	...
	data2 = np.append(data2,[float(yval)])
	temp+=1
norm =max(abs(data1))
norm2 = max(abs(data2))
...
for j in range(1,100):
	...
	while var<100:
		if j == 1:
			...
			y_value = np.append(y_value,[float(yval1)])
		#time.sleep(0.03)
		#x_value[:] = [i / 200 for i in x_value]
		#training the data
		if var > 1:
			u = x_value[var-1]/norm
			u2 = y_value[var-1]/norm2
			#print " the u is ", u
			x = (1-a)*x + a*tanh( dot (Win , u ) + dot( W, x) + dot(Wfb, (x_value[var-2]/norm)) )
			x2 = (1-a)*x2 + a*tanh( dot (Win2 , u2 ) + dot( W2, x2) + dot(Wfb2, (y_value[var-2]/norm2)) )
			ypre = dot(Wout,x)
			ypre2 = dot(Wout2,x2)    
			Wdel = 0.01*((x_value[var]/norm) - ypre)*(x.T)
			Wdel2 = 0.01*((y_value[var]/norm2) - ypre2)*(x2.T)
			Wout = Wout + Wdel
			Wout2 = Wout2+Wdel2
		var+=1	
...
for t in range(1,testLen):
	...
	u2 = data2[t-1]/norm2
	if t<21:
		  x = (1-a)*x + a*tanh( dot (Win , u ) + dot( W, x) + dot(Wfb, data1[t]/norm) )
		  x2 = (1-a)*x2 + a*tanh( dot (Win2 , u2 ) + dot( W2, x2) + dot(Wfb2, data2[t]/norm2) )
	else:
		x = (1-a)*x + a*tanh( dot (Win , u ) + dot( W, x) + dot(Wfb,y) )
		x2 = (1-a)*x2 + a*tanh( dot (Win2 , u2 ) + dot( W2, x2) + dot(Wfb2,y2) ) 
	y = dot(Wout,x)
	y2 = dot(Wout2,x2)
	Y[:,t]= y
	Y2[:,t] = y2
...
plt2.ion()
plt2.plot(Y2.T,"b")
plt2.plot(data2/norm2,"m")
plt2.show
plt2.savefig("plot4.png",format = 'png')
plt2.close
